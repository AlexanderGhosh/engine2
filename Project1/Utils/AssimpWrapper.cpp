...
Primative::Mesh* FileReaders::AssimpWrapper::processMeshVertices(aiMesh* mesh, const aiScene* scene)
{
    ...
    /*
    // process materials
    aiMaterial* albedo = scene->mMaterials[mesh->mMaterialIndex];
    ...
    // 1. diffuse maps
    vector<Texture> diffuseMaps = loadMaterialTextures(albedo, aiTextureType_DIFFUSE, "texture_diffuse");
    textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
    // 2. specular maps
    vector<Texture> specularMaps = loadMaterialTextures(albedo, aiTextureType_SPECULAR, "texture_specular");
    textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
    // 3. normal maps
    std::vector<Texture> normalMaps = loadMaterialTextures(albedo, aiTextureType_HEIGHT, "texture_normal");
    textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
    // 4. height maps
    std::vector<Texture> heightMaps = loadMaterialTextures(albedo, aiTextureType_AMBIENT, "texture_height");
